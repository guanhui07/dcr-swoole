...
namespace DcrSwoole\Permission\Adapter;
...
use Closure;
use DcrSwoole\Permission\Model\LaravelRuleModel;
...
class LaravelDatabaseAdapter implements Adapter, UpdatableAdapter, BatchAdapter, FilteredAdapter
{
    ...
    public function savePolicyLine(string $ptype, array $rule): void
    {
        ...
        $this->model->newQuery()->create($col);
    }
    ...
    public function addPolicies(string $sec, string $ptype, array $rules): void
    {
        ...
        $this->model->newQuery()->insert($cols);
        (new LaravelRuleModel)->fireModelEvent('saved');
    }
    ...
    {
        $instance = $this->model->newQuery()->where('ptype', $ptype);
        foreach ($rule as $key => $value) {
            $instance->where('v' . strval($key), $value);
        }
        $instance->delete();
        (new LaravelRuleModel)->fireModelEvent('deleted');
    }
    ...
    public function loadFilteredPolicy(Model $model, $filter): void
    {
        $instance = $this->model;
        if (is_string($filter)) {
            $instance = $instance->newQuery()->whereRaw($filter);
        } elseif ($filter instanceof Filter) {
            foreach ($filter->p as $k => $v) {
                $where[$v] = $filter->g[$k];
                $instance = $instance->newQuery()->where($v, $filter->g[$k]);
            }
        } elseif ($filter instanceof Closure) {
            $instance = $instance->newQuery()->where($filter);
        } else {
        ...
        if ($rows) {
            foreach ($rows as $row) {
                $row = array_filter($row, static function($value) { return !is_null($value) && $value !== ''; });
